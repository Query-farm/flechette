...
function writeInt64(buf, index, value) {
 ...
 */
function setCompressionCodec(type, codec) {
...
function createTable(data, options = {}) {
  ...
  const processDict = (dict) => {
    ...
    const batch = visit$1(type, context({ ...dictData, body }));
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    fields.forEach((f, idx) => cols[idx].add(visit$1(f.type, ctx)));
  }
 ...
 * @returns {number}
 */
function encodeRecordBatch(builder, batch, compression) {
 ...
 * @returns {number}
 */
function encodeDictionaryBatch(builder, dictionaryBatch, compression) {
 ...
 */
function encodeIPC(data, { sink, format = STREAM, codec } = {}) {
...
function assembleContext(codec) {
  ...
  return {
    ...
    buffer(b) {
      ...
      buffers.push(buf);
      byteLength += ((length + 7) & -8); // align to padding
    },
    /**
     * @param {number} length
     */
 ...
 *  or null (if using a sink that writes bytes elsewhere).
 */
function tablesToIPC(tables, options) {
...
function concatTables(tables) {
  ...
  return new Table(head.schema, mergedChildren);
}
...
export { Batch, Column, CompressionType, DateUnit, Endianness, IntervalUnit, Precision, RowIndex, Table, TimeUnit, Type, UnionMode, Version, batchType, binary, binaryView, bool, columnFromArray, columnFromValues, concatTables, date, dateDay, dateMillisecond, decimal, decimal128, decimal256, decimal32, decimal64, dictionary, dictionaryContext, duration, field, fixedSizeBinary, fixedSizeList, float, float16, float32, float64, getCompressionCodec, int, int16, int32, int64, int8, interval, largeBinary, largeList, largeListView, largeUtf8, list, listView, map, nullType, runEndEncoded, setCompressionCodec, struct, tableFromArrays, tableFromColumns, tableFromIPC, tableToIPC, tablesToIPC, time, timeMicrosecond, timeMillisecond, timeNanosecond, timeSecond, timestamp, uint16, uint32, uint64, uint8, union, utf8, utf8View };
